	...
	(async () => {
		try {
			...
			if (supabase) {
				...
				if (error) {
					logger.error('[server] ❌ Database warm-up failed', {
						error: error.message,
						code: error.code,
						details: error.details
					});
					// In test mode, don't fail - allow server to start for testing
					if (process.env.NODE_ENV === 'test') {
						logger.warn('[server] ⚠️ Database warm-up failed in test mode - continuing anyway');
					} else {
